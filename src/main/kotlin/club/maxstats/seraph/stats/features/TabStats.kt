...
class TabStatsGui(minecraft: Minecraft, guiIngame: GuiIngame) : GuiPlayerTabOverlay(minecraft, guiIngame) {
    ...
    private fun renderStatTab(screenWidth: Int, scoreboard: Scoreboard?, objective: ScoreObjective?) {
        ...
        val playerList = playerOrdering.sortedCopy(netHandler.playerInfoMap.values).take(40)
        ...
        val statTitlesList = statTitles 
        statTitlesList.forEach { title ->
            mc.fontRendererObj.drawStringWithShadow(
                "${ChatColor.BOLD}$title",
                xSpacer,
                startingY + (entryHeight / 2 - 4),
                Color.white.toRGBA()
            )
            xSpacer += mc.fontRendererObj.getStringWidth("${ChatColor.BOLD}$title") + 10
        }

        var ySpacer = startingY + entryHeight + 1
        playerList.forEachIndexed { index, playerInfo ->
            xSpacer = startingX
            ...
            val playerName = getPlayerName(playerInfo)
            ...
            if (mc.isIntegratedServerRunning || mc.netHandler.networkManager.isencrypted) {
                ...
                if (playerInfo.locationSkin != null) {
            ...
            if (playerInfo.gameType == WorldSettings.GameType.SPECTATOR) {
                // idk
            } else {
